...
AUDIT_TOOL_REQUIREMENTS = REPO_ROOT / "scripts" / "release-audit-tool-requirements.txt"
...
def _validate_audit_tool_requirements() -> str:
    """Validate the committed, hash-locked pip-audit runtime closure."""

    try:
        text = AUDIT_TOOL_REQUIREMENTS.read_text(encoding="utf-8")
    except OSError:
        raise DependencyAuditError("audit runtime closure could not be read") from None

    active_versions: dict[str, str] = {}
    for entry in _requirement_entries(text, role="audit runtime closure"):
        name, version, is_active, _ = _parse_requirement_entry(
            entry,
            role="audit runtime closure",
        )
        if not is_active:
            continue
        previous_version = active_versions.get(name)
        if previous_version is not None and previous_version != version:
            raise DependencyAuditError(
                f"audit runtime closure contains multiple active versions for {name}"
            )
        active_versions[name] = version

    expected_name, expected_version, _, _ = _parse_requirement_entry(
        f"{AUDIT_TOOL} --hash=sha256:{'0' * 64}",
        role="audit runtime identity",
    )
    if active_versions.get(expected_name) != expected_version:
        raise DependencyAuditError(f"audit runtime closure must pin {AUDIT_TOOL}")
...
def verify_dependency_audit(
    ...
    *,
    ...
    audit_tool_digest = _validate_audit_tool_requirements()
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="localql-dependency-audit-") as run_temp_text:
                ...
                for command_index, (command, role, output) in enumerate(
                    ...
                    _require_prepared_directory(prepared)
                    _revalidate_frozen_lock(lock)
                    if _validate_audit_tool_requirements() != audit_tool_digest:
                        raise DependencyAuditError(
                            "audit runtime closure changed during collection"
                        )
                    try:
                        _run_checked(
                            command,
            ...
            raise DependencyAuditError("isolated dependency-audit runtime cleanup failed") from None
