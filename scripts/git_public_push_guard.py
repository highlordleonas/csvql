...
TAG_AUTHORIZATION_KEYS = BASE_AUTHORIZATION_KEYS | {"tag_object_oid", "peeled_commit_oid"}
...
def _reject_duplicate_authorization_keys(pairs: list[tuple[str, object]]) -> dict[str, object]:
    payload: dict[str, object] = {}
    for key, value in pairs:
        if key in payload:
            raise GuardError(f"duplicate authorization key: {key}")
        payload[key] = value
    return payload


def _validate_authorization_invariants(authorization: Authorization) -> None:
    require(
        type(authorization.schema) is int and authorization.schema == 1,
        "authorization schema must be integer 1",
    )
    require(
        isinstance(authorization.repository, str)
        and authorization.repository == CANONICAL_REPOSITORY,
        "authorization schema or repository does not match LocalQL public push policy",
    )
    require(
        isinstance(authorization.operation, str) and authorization.operation in ALLOWED_OPERATIONS,
        "authorization operation is not supported",
    )
    require(
        isinstance(authorization.destination_ref, str),
        "authorization destination_ref must be a string",
    )
    for key, value in (
        ("new_oid", authorization.new_oid),
        ("expected_remote_oid", authorization.expected_remote_oid),
    ):
        require(
            isinstance(value, str) and OID_RE.fullmatch(value) is not None,
            f"authorization {key} must be a lowercase 40-character object ID",
        )

    if authorization.operation == "create_annotated_tag":
        for tag_key, tag_value in (
            ("tag_object_oid", authorization.tag_object_oid),
            ("peeled_commit_oid", authorization.peeled_commit_oid),
        ):
            require(
                isinstance(tag_value, str) and OID_RE.fullmatch(tag_value) is not None,
                f"authorization {tag_key} must be a lowercase 40-character object ID",
            )
    else:
        require(
            authorization.tag_object_oid is None and authorization.peeled_commit_oid is None,
            "branch authorizations must not include tag object fields",
        )


def parse_authorization(raw: str | None) -> Authorization:
    if raw is None:
        raise GuardError("public push requires inline LOCALQL_PUBLIC_PUSH_AUTHORIZATION JSON")
    try:
        payload = json.loads(raw, object_pairs_hook=_reject_duplicate_authorization_keys)
    except json.JSONDecodeError as error:
        raise GuardError("LOCALQL_PUBLIC_PUSH_AUTHORIZATION is not valid JSON") from error
    if not isinstance(payload, dict):
        raise GuardError("LOCALQL_PUBLIC_PUSH_AUTHORIZATION must be a JSON object")
    operation = payload.get("operation")
    expected_keys = (
        TAG_AUTHORIZATION_KEYS if operation == "create_annotated_tag" else BASE_AUTHORIZATION_KEYS
    )
    if set(payload) != expected_keys:
        raise GuardError(f"authorization keys must be exactly {sorted(expected_keys)}")
    ...
    _validate_authorization_invariants(authorization)
...
def validate_public_push(
    ...
    require(len(updates) == 1, "public push authorization permits exactly one ref transition")
    update = updates[0]
    require(
    ...
    require(
        update.destination_ref == authorization.destination_ref,
        "authorization destination ref does not match the public push",
    )
    require(
        update.source_oid == authorization.new_oid,
        "authorization new object does not match the public push",
    )
    require(
        update.remote_oid == authorization.expected_remote_oid,
        "authorization remote object does not match the public push",
    )

    if authorization.operation == "create_branch":
        require(
    ...
    elif authorization.operation == "update_branch":
        require(
            update.destination_ref.startswith("refs/heads/"),
        ...
        require(
            authorization.tag_object_oid == update.source_oid,
            "authorization tag object does not match",
        )
        require(
        ...
        require(
            git.run("rev-parse", f"{update.source_oid}^{{commit}}")
            == authorization.peeled_commit_oid,
            "authorization peeled commit does not match the tag",
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if normalize_public_repository(remote_url) is not None:
            authorization = parse_authorization(os.environ.get("LOCALQL_PUBLIC_PUSH_AUTHORIZATION"))
        validate_public_push(remote_url, updates, authorization, SubprocessGitRunner())
