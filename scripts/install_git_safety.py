...
def append_exclusion_atomically(exclude_path: Path) -> None:
    ...
    if LOCAL_EXCLUSION not in lines:
        lines.append(LOCAL_EXCLUSION)
    content = "\n".join(lines) + "\n"
...
def inspect_repository(repo: Path) -> Inspection:
    ...
    return Inspection(
        ...
        local_excluded=LOCAL_EXCLUSION in exclude_lines,
...
def _print_inspection(inspection: Inspection) -> None:
    ...
    print(f"local exclusion: {'present' if inspection.local_excluded else 'missing'}")
