...
def test_build_commands_uses_exact_locked_core_and_tui_contract(tmp_path: Path) -> None:
    ...
    assert verifier.build_commands(evidence_dir) == (
        ...
        (
            ...
            "--no-project",
            "--with-requirements",
            str(verifier.AUDIT_TOOL_REQUIREMENTS),
            "pip-audit",
            "--requirement",
            str(core_path),
            "--no-deps",
...
def test_audit_runtime_closure_is_committed_and_hash_locked() -> None:
    closure = verifier.AUDIT_TOOL_REQUIREMENTS

    assert closure == SCRIPTS_DIR / "release-audit-tool-requirements.txt"
    text = closure.read_text(encoding="utf-8")
    entries = verifier._requirement_entries(text, role="audit runtime closure")
    active = {
        verifier._parse_requirement_entry(entry, role="audit runtime closure")[0]
        for entry in entries
    }

    assert "pip-audit" in active
    assert all("--hash=sha256:" in entry for entry in entries)
    assert len(verifier._validate_audit_tool_requirements()) == 64


def test_audit_runtime_closure_rejects_missing_pip_audit(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    closure = tmp_path / "audit-tool-requirements.txt"
    closure.write_text(_hashed_requirement("alpha==1.0"), encoding="utf-8")
    monkeypatch.setattr(verifier, "AUDIT_TOOL_REQUIREMENTS", closure)

    with pytest.raises(verifier.DependencyAuditError, match="must pin"):
        verifier._validate_audit_tool_requirements()
...
def test_every_command_is_bounded_checked_sanitized_and_runs_at_repo_root(
    ...
    for call in runner.calls:
        ...
        assert environment["UV_REQUIRE_HASHES"] == "1"
...
def test_isolated_runtime_cleanup_failure_cannot_publish_success(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    def fail_isolated_runtime_cleanup(
        temporary_directory: tempfile.TemporaryDirectory[str],
    ) -> None:
        nonlocal cleanup_calls
        cleanup_calls += 1
        raise OSError("forced isolated runtime cleanup failure")

    monkeypatch.setattr(
        verifier.tempfile.TemporaryDirectory,
        "cleanup",
        fail_isolated_runtime_cleanup,
    )
    runner = RecordingRunner()
    evidence_dir = tmp_path / "dependency-audit"
    ...
    assert excinfo.value.code == 2
    assert captured.out == ""
    assert "isolated dependency-audit runtime cleanup failed" in captured.err
    assert cleanup_calls == 1
    assert len(runner.calls) == 4
    assert not (evidence_dir / "manifest.json").exists()


def test_each_run_uses_isolated_temporary_uv_runtime_directories(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
