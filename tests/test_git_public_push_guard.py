...
def branch_approval(operation: str, destination_ref: str, new_oid: str, old_oid: str) -> str:
    return json.dumps(
        {
            "schema": 1,
...
def test_public_branch_creation_requires_one_exact_transition() -> None:
    ...
    approval = parse_approval(
        branch_approval("create_branch", update.destination_ref, new_oid, ZERO_OID)
    )
    validate_public_push(CANONICAL_HTTPS_URL, (update,), approval, FakeGitRunner())
...
def test_public_branch_update_requires_the_exact_remote_object() -> None:
    ...
    approval = parse_approval(
        branch_approval("update_branch", update.destination_ref, new_oid, old_oid)
    )
    validate_public_push(CANONICAL_HTTPS_URL, (update,), approval, FakeGitRunner())
...
def test_annotated_tag_creation_binds_tag_object_and_peeled_commit() -> None:
    ...
    validate_public_push(CANONICAL_HTTPS_URL, (update,), approval, git)
...
def test_public_push_requires_inline_approval() -> None:
...
@pytest.mark.parametrize(
    ("mutate", "message"),
    [
        (lambda payload: payload.update(extra="value"), "approval keys must be exactly"),
        (lambda payload: payload.pop("new_oid"), "approval keys must be exactly"),
...
def test_parse_approval_rejects_unknown_missing_or_mismatched_schema_fields(
    mutate: Callable[[dict[str, object]], None], message: str
) -> None:
    payload = json.loads(
        branch_approval("create_branch", "refs/heads/release/x", "1" * 40, ZERO_OID)
    )
    assert isinstance(payload, dict)
    mutate(payload)

    with pytest.raises(GuardError, match=message):
        parse_approval(json.dumps(payload))
...
def test_parse_approval_rejects_missing_or_malformed_json(raw: str | None, message: str) -> None:
    with pytest.raises(GuardError, match=message):
        parse_approval(raw)


def test_parse_approval_rejects_duplicate_json_keys() -> None:
    raw = (
    ...
    with pytest.raises(GuardError, match="duplicate approval key: schema"):
        parse_approval(raw)


@pytest.mark.parametrize("null_key", ["tag_object_oid", "peeled_commit_oid"])
def test_parse_approval_rejects_null_annotated_tag_object_ids(null_key: str) -> None:
    payload = json.loads(annotated_tag_approval("refs/tags/v1.0.2", "3" * 40, ZERO_OID, "2" * 40))
    payload[null_key] = None

    with pytest.raises(GuardError, match=rf"approval {null_key} must be a lowercase"):
        parse_approval(json.dumps(payload))


@pytest.mark.parametrize(
    ("approval_raw", "message"),
    [
        (
            branch_approval("create_branch", "refs/heads/release/other", "1" * 40, ZERO_OID),
            "destination ref does not match",
        ),
        (
            branch_approval("create_branch", "refs/heads/release/x", "2" * 40, ZERO_OID),
            "new object does not match",
        ),
        (
            branch_approval("create_branch", "refs/heads/release/x", "1" * 40, "2" * 40),
            "remote object does not match",
        ),
    ],
)
def test_public_push_rejects_manifest_transition_mismatches(
    approval_raw: str, message: str
) -> None:
    update = RefUpdate("refs/heads/release/x", "1" * 40, "refs/heads/release/x", ZERO_OID)

    with pytest.raises(GuardError, match=message):
        validate_public_push(
            CANONICAL_HTTPS_URL, (update,), parse_approval(approval_raw), FakeGitRunner()
        )
...
def test_public_branch_operation_must_match_create_or_update_state(
    ...
    approval = parse_approval(
        branch_approval(operation, update.destination_ref, new_oid, remote_oid)
    )
...
def test_public_tag_update_is_prohibited() -> None:
    ...
    approval = parse_approval(
        annotated_tag_approval(update.destination_ref, tag_oid, old_oid, "2" * 40)
    )
...
def test_public_lightweight_tag_is_prohibited() -> None:
    ...
    update = RefUpdate("refs/tags/v1.0.2", tag_oid, "refs/tags/v1.0.2", ZERO_OID)
    approval = parse_approval(
        annotated_tag_approval(update.destination_ref, tag_oid, ZERO_OID, commit_oid)
    )
...
def test_public_tag_peeled_commit_must_match_manifest() -> None:
    ...
    update = RefUpdate("refs/tags/v1.0.2", tag_oid, "refs/tags/v1.0.2", ZERO_OID)
    approval = parse_approval(
        annotated_tag_approval(update.destination_ref, tag_oid, ZERO_OID, "2" * 40)
    )
    git = FakeGitRunner(
        {
            ("cat-file", "-t", tag_oid): "tag",
...
def test_wildcard_expansion_cannot_authorize_two_ref_transitions() -> None:
    ...
    with pytest.raises(GuardError, match="exactly one ref transition"):
        validate_public_push(CANONICAL_HTTPS_URL, updates, approval, FakeGitRunner())
...
def test_non_public_repository_returns_without_approval_or_git_calls() -> None:
...
DIRECT_BRANCH_APPROVAL = Approval(
    schema=1,
    repository="highlordleonas/csvql",
...
@pytest.mark.parametrize(
    ("approval", "message"),
    [
        (replace(DIRECT_BRANCH_APPROVAL, schema=2), "approval schema must be integer 1"),
        (replace(DIRECT_BRANCH_APPROVAL, schema=True), "approval schema must be integer 1"),
        (
            replace(DIRECT_BRANCH_APPROVAL, repository="other/csvql"),
            "repository does not match",
        ),
        (
            replace(DIRECT_BRANCH_APPROVAL, operation="unsupported"),
            "approval operation is not supported",
        ),
        (
            replace(DIRECT_BRANCH_APPROVAL, destination_ref=42),  # type: ignore[arg-type]
            "approval destination_ref must be a string",
        ),
        (
            replace(DIRECT_BRANCH_APPROVAL, new_oid="A" * 40),
            "approval new_oid must be a lowercase",
        ),
        (
            replace(DIRECT_BRANCH_APPROVAL, expected_remote_oid="g" * 40),
            "approval expected_remote_oid must be a lowercase",
        ),
        (
            replace(DIRECT_BRANCH_APPROVAL, tag_object_oid="3" * 40),
            "branch approvals must not include tag object fields",
        ),
        (
            replace(DIRECT_BRANCH_APPROVAL, peeled_commit_oid="2" * 40),
            "branch approvals must not include tag object fields",
        ),
    ],
)
def test_validate_public_push_revalidates_direct_branch_approval_invariants(
...
@pytest.mark.parametrize(
    ...
    [
        (
            replace(DIRECT_TAG_APPROVAL, tag_object_oid=None),
            "approval tag_object_oid must be a lowercase",
        ),
        (
            replace(DIRECT_TAG_APPROVAL, peeled_commit_oid=None),
            "approval peeled_commit_oid must be a lowercase",
        ),
        (
            replace(DIRECT_TAG_APPROVAL, tag_object_oid="A" * 40),
            "approval tag_object_oid must be a lowercase",
        ),
        (
            replace(DIRECT_TAG_APPROVAL, peeled_commit_oid="g" * 40),
            "approval peeled_commit_oid must be a lowercase",
        ),
    ],
)
def test_validate_public_push_revalidates_direct_tag_approval_invariants(
    approval: Approval, message: str
) -> None:
...
def test_subprocess_git_runner_redacts_captured_failure_and_environment(
    ...
    monkeypatch.setenv("LOCALQL_PUBLIC_PUSH_APPROVAL", secret_manifest)
...
def run_guard_cli(
    ...
    update: RefUpdate,
    *,
    approval_raw: str | None,
) -> subprocess.CompletedProcess[str]:
    environment = {}
    if approval_raw is not None:
        environment["LOCALQL_PUBLIC_PUSH_APPROVAL"] = approval_raw
    protocol_line = (
...
def test_cli_allows_one_approved_transition_with_direct_url_arguments() -> None:
    new_oid = "1" * 40
    update = RefUpdate("refs/heads/release/x", new_oid, "refs/heads/release/x", ZERO_OID)
    approval_raw = branch_approval("create_branch", update.destination_ref, new_oid, ZERO_OID)
...
def test_cli_classifies_canonical_repository_by_url_not_misleading_remote_name() -> None:
    ...
    assert approval_raw not in completed.stderr
...
def test_cli_rejection_never_prints_the_inline_manifest() -> None:
    update = RefUpdate("refs/heads/release/x", "1" * 40, "refs/heads/release/x", ZERO_OID)
    approval_raw = branch_approval("create_branch", update.destination_ref, "2" * 40, ZERO_OID)

    completed = run_guard_cli("origin", CANONICAL_HTTPS_URL, update, approval_raw=approval_raw)
...
def test_cli_rejects_public_transition_when_environment_approval_is_missing() -> None:
    update = RefUpdate("refs/heads/release/x", "1" * 40, "refs/heads/release/x", ZERO_OID)

    completed = run_guard_cli("origin", CANONICAL_HTTPS_URL, update, approval_raw=None)

    assert completed.returncode == 1
    assert update.destination_ref in completed.stderr
    assert "requires inline LOCALQL_PUBLIC_PUSH_APPROVAL JSON" in completed.stderr
    assert "Local hooks can be bypassed; GitHub hosted rules are authoritative." in completed.stderr
...
def test_cli_rejects_canonical_query_and_fragment_urls(remote_url: str) -> None:
    ...
    completed = run_guard_cli("origin", remote_url, update, approval_raw=None)
...
def test_cli_rejects_malformed_url_without_traceback(malformed_url: str) -> None:
    ...
    completed = run_guard_cli("origin", malformed_url, update, approval_raw=None)
