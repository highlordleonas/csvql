...
EXPECTED_WHEEL = "localql-1.2.0-py3-none-any.whl"
EXPECTED_SDIST = "localql-1.2.0.tar.gz"
DIST_INFO = "localql-1.2.0.dist-info"
SDIST_ROOT = "localql-1.2.0"
SOURCE_COMMIT = "a" * 40
TAG_OBJECT = "b" * 40
...
def wheel_entries(
    ...
    *,
    ...
    entries: list[tuple[str | zipfile.ZipInfo, bytes | str]] = [
        ("csvql/__init__.py", '__version__ = "1.2.0"\n'),
...
def invoke_wheel_archive_interface(
    ...
    if interface == "artifact-pair":
        ...
        module.verify_artifact_pair(module.ArtifactSet(wheel, sdist), "1.2.0")
        return
...
def test_artifact_pair_accepts_exact_metadata_and_entry_point(tmp_path: Path) -> None:
    ...
    assert module.verify_artifact_pair(module.ArtifactSet(wheel, sdist), "1.2.0") is None
...
@pytest.mark.parametrize(
    "arguments",
    [
        ["inspect", "dist", "--expected-version", "1.2.0", "--manifest", "manifest.json"],
        ["create-manifest", "dist", "--expected-version", "1.2.0"],
        ["verify-manifest", "dist", "--expected-version", "1.2.0", "--rebuilt-wheel", "wheel"],
        ["verify-rebuild", "dist", "--expected-version", "1.2.0"],
    ],
)
