...
Status: LocalQL distribution alias accepted for `v1.0.0`; full local candidate
proof refreshed at `fea95f5 release: adopt localql distribution alias`
...
Current status is LocalQL distribution alias accepted for `v1.0.0`; full local
candidate proof has been refreshed before external release action.

The tracked proof packet is
[`docs/release-candidate-proof-2026-07-02.md`](../release-candidate-proof-2026-07-02.md).
It records a `release-candidate eligible` local assessment for proof target
`fea95f5 release: adopt localql distribution alias`, including `localql`
wheel/sdist artifact proof while preserving the installed `csvql` command,
Python import package, and `.csvql.yml` config contract.

The next task after local proof is explicit user approval for any external
release action: creating a tag, creating a GitHub release, publishing to PyPI,
