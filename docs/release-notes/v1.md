...
Status: local `v1.0.0` release state
...
- `pyproject.toml` with package version `1.0.0`
...
Candidate decision: local `v1.0.0` release state prepared for final proof.
...
Current assessment is local `v1.0.0` release state prepared for final proof.
...
- `env UV_CACHE_DIR=/private/tmp/uv-cache-csvql-release-20260701 uv run python scripts/verify_release_readiness.py --work-dir output/release-readiness-v1`
