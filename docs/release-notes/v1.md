...
command, baseline transcripts, passed items, and blockers. The approved TUI release-proof target covers macOS Terminal, Windows Terminal, and one normal Linux desktop terminal. VS Code integrated terminal, iTerm2, and tmux/SSH are out of scope for this release lane.
...
- the manual v1 QA matrix is recorded for the candidate state
- the TUI QoL QA gate passes on macOS Terminal, Windows Terminal, and one
  normal Linux desktop terminal with a recorded run id, media artifact paths,
  baseline transcripts, source access method, commit verification command, and
