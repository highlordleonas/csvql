...
Status: LocalQL distribution alias accepted for `v1.0.0`; current work is in
`v1-hardening` while pre-release blocker fixes refresh the support and proof
contract, including Python 3.13 and Python 3.14 support proof.
...
does not publish to PyPI, create a GitHub release, upload artifacts, or claim
`v1-stable`.
...
Current status is LocalQL distribution alias accepted for `v1.0.0`, with current
work in `v1-hardening` while pre-release blockers and Python support proof are
refreshed. Public docs keep the release proof procedure in
[Release readiness](../release-readiness.md) instead of tracking raw local proof
packets in the public branch.

The next task is refreshed release-candidate eligibility proof on the final
hardening `HEAD`, then explicit user approval for any further external release
action: creating a GitHub release, publishing to PyPI, uploading artifacts, or
claiming externally published `v1-stable`.
