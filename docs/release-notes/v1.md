...
Status: LocalQL distribution alias accepted for `v1.0.0`; candidate commit
...
Current status is LocalQL distribution alias accepted for `v1.0.0`, with
candidate commit `a0f3146` assessed as `release-candidate eligible`. Public docs
keep the release proof procedure in [Release readiness](../release-readiness.md);
the local evidence packet remains ignored proof output rather than a committed
release artifact.

The next task is explicit user approval for any further external release action:
creating a GitHub release, publishing to PyPI, uploading artifacts, or claiming
